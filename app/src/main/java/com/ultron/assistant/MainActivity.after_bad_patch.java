...
public class MainActivity extends Activity {
    ...
    private android.media.ImageReader visionImageReader;
    private android.view.Surface visionCameraSurface;
    ...
    private void openHiddenVisionCamera() {
        if (checkSelfPermission(Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED) {
            requestPermissions(
        ...
        try {
            ...
            if (selectedCamera == null) {
                status.setText("Rear camera not available");
                return;
            }
            ...
            visionImageReader.setOnImageAvailableListener(
                    reader -> {
            ...
            manager.openCamera(
                    ...
                    new CameraDevice.StateCallback() {
                        ...
                        public void onOpened(CameraDevice device) {
                            ...
                            try {
                                CameraDevice.CaptureRequest.Builder request =
                                        camera.createCaptureRequest(
                                                CameraDevice.TEMPLATE_PREVIEW
                                        );

                                request.addTarget(visionCameraSurface);
            ...
            cameraLive = false;
            status.setText("Hidden camera access failed");
        }
    }
    ...
    private void closeCamera() {
        ...
        if (visionCameraSurface != null) {
            visionCameraSurface.release();
            visionCameraSurface = null;
        }

        if (visionImageReader != null) {
            visionImageReader.close();
            visionImageReader = null;
        }
